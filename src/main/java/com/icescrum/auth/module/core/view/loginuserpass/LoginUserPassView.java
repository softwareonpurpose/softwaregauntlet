...
public class LoginUserPassView extends UiView implements LoginUserPassViewCalibratable {
    ...
    protected boolean confirmElementStates() {
        boolean confirmed = WebUiHost.getInstance().getAddress().contains(URI_VIEW);
    ...
    private UiElement getPasswordElement() {
        return UiElement.getInstance("'Password' field", UiLocatorType.ID, "user_pass", this.getElement());
    }

    private UiElement getLoginButtonElement() {
        return UiElement.getInstance("'Login' button", UiLocatorType.NAME, "login", this.getElement());
    }
}
