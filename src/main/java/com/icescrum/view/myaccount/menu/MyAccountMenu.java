...
public class MyAccountMenu extends UiRegion implements MyAccountMenuCalibratable {
    ...
    public LandingView clickLogout() {
        UiElement.getInstance("'Logout' link", UiLocatorType.TAG, "a", getLogoutElement()).click();
        return UiView.expect(LandingView.class);
    }

    private UiElement getLogoutElement() {
        return UiElement.getInstance("'Logout' item", UiLocatorType.CLASS, "woocommerce-MyAccount-navigation-link--customer-logout", this.getElement());
    }
}
